...
function configDidUpdate() {
	...
	if (skyLightingSelector() === SKY_LIGHT_NONE) {
		// appNodes.canvasContainer.style.backgroundColor = '#000'; //背景颜色
	}
...
// Map of color codes to their index.vue in the array. Useful for quickly determining if a color has already been updated in a loop.
...
function updateGlobals(timeStep, lag) {
	...
	if (store.state.config.autoLaunch) {
		...
		if (autoLaunchTime <= 0) {
			autoLaunchTime = startSequence() * 3;      // 烟花的密集程度，改变时间
			// autoLaunchTime = autoLaunchTime>40?40:autoLaunchTime;
		}
	}
}
...
if (IS_HEADER) {
	...
	setLoadingStatus('2024 is coming, please wait...');
	setTimeout(() => {
