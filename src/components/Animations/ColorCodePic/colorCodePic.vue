<template>
  <div class="code-container-bg">
    <div class="code-container">
      <input class="sig" value="// 小疼 💜">
      <div class="glow-container">
        <div class="augs" data-augmented-ui></div>
      </div>
      <section class="augs bg" >
        <button class="dots" onclick="changeMode(this)" title="(click to change) Current Mode: css"></button>
        <input class="title">
...
function svgParticles(svg_url) {
    ...
    async function init() {
        particlesArray = [];
        const svgPaths = await loadSVG(svg_url);
        let allPositions = [];
...
svgParticles('/src/assets/imgs/RoitGames.svg');
...
#canvas {
  cursor: url('@/assets/imgs/roit.svg'), auto;
}
...
import XiaoTeng from "@/components/Animations/XiaoTeng/xiaoTeng.vue";

onMounted(() => {
  add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/codemirror.js');

  setTimeout(() => {
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/javascript/javascript.js');
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/css/css.js');
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/scroll/simplescrollbars.min.js');
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/markdown/markdown.min.js');
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/htmlmixed/htmlmixed.min.js');
    add_script('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/mode/xml/xml.js');
  }, 100);
  setTimeout(() => {
    add_script('/src/components/Animations/ColorCodePic/colorCodePic.js');
    ...
    const startAutoDelete = () => {
      deleteInterval = setInterval(() => {
        ...
        if (codeMirrorCode && codeMirrorCode.firstChild) {
          delete_count ++;
          codeMirrorScroll.scrollTop = delete_count *20;
        } else {
          console.log('CodeMirror code deleted successfully');
...
@import url('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.35.0/codemirror.css');
@import url('https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.58.1/addon/scroll/simplescrollbars.min.css');
body {
  background: #000;
}
...
.code-container-bg {
  ...
  --code-box-color: #d3d3f8;
  //background: black; height: 600px;  width: 800px;
...
input.sig {
  ...
  right: 90px;
  z-index: 999;
  color: rgba(68, 66, 68, 0.5);
  text-shadow: 0 0 12px var(--code-box-color);
  ...
  text-align: right;
  letter-spacing: 0.1em;
  font-family: 'QingNiaoHuaGuangFanXingKai';
}
...
.bg {
  ...
  linear-gradient(to bottom, var(--code-box-color) 75%, var(--code-box-color));
}
.bg::after{
  content:'';
  height:50%;
